...
- 适配新版引擎（≥ `0.1.2-rc.1`）的**启动令牌浏览器认证**：引擎启动时会打印带 `?token=` 的认证 URL，不带令牌的浏览器请求一律返回 401（`dsh web authentication required`）。桌面壳现改为**解析引擎 stdout 的 `dsh web:` URL 行**，并让窗口直接加载带令牌的认证地址（`engine.js` / `main.js`），不再打开裸地址
- 引擎就绪判定改由 URL 打印行驱动（该行即引擎自身的就绪信号），`healthCheck` 同时识别认证 401 响应，端口占用/接管判定不再误判新引擎
